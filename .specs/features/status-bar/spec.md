...
| Fetch staleness | `FETCH_HEAD`'s modification time in the repo's common git dir, rendered with the existing `relative-time.ts`; never fetched reads `never` | The standard proxy; a worktree has no `FETCH_HEAD` of its own | y |
...
| Sync semantics | `pull --ff-only` then `push` | Same policy as the app's existing base refresh (`merge --ff-only`); it cannot produce a conflict or a merge commit, and a divergence refuses with a readable message | y |
...
| Error reporting | Git's first stderr line (`gitFailureLine`, `worktree-manager.ts:365`) inline in the popover, which stays open | The error belongs beside the button that caused it and must not disappear on a timer | y |
...
17. WHEN the user activates Sync THEN the app SHALL run `pull --ff-only` followed by `push` <!-- event-driven -->
